...
const HeaderShell = styled.div`
    ...
    &.click-through .button:hover,
    &.click-through .icon-button:hover {
...
const navigationViews = new Set(['customize', 'help', 'history', 'advanced']);
...
const AppHeader = ({
    ...
    const elapsedTime = useMemo(() => {
        ...
        return `${seconds}s`;
    }, [currentView, startTime, tick]);

    const viewTitle = titles[currentView] || 'Cheating Daddy';
    const isNavView = navigationViews.has(currentView);
    const isMac = window.cheddar?.isMacOS || navigator.platform.toUpperCase().includes('MAC');

    return (
        <HeaderShell className={isClickThrough ? 'click-through' : ''}>
            <div className="header">
                <div className="header-title">{viewTitle}</div>
                <div className="header-actions">
                    ...
                    {currentView === 'main' && (
                        <>
                            <button className="icon-button" onClick={onHistoryClick} aria-label="History" type="button">
                                <svg
                                    width="24px"
                                    height="24px"
                                    strokeWidth="1.7"
                            ...
                            {advancedMode && (
                                <button className="icon-button" onClick={onAdvancedClick} title="Advanced Tools" type="button">
                                    <svg
                                        width="24px"
                                        strokeWidth="1.7"
                                        height="24px"
                            ...
                            <button className="icon-button" onClick={onCustomizeClick} title="Customize" type="button">
                                <svg
                                    width="24px"
                                    height="24px"
                                    strokeWidth="1.7"
                            ...
                            <button className="icon-button" onClick={onHelpClick} title="Help" type="button">
                                <svg
                                    width="24px"
                                    height="24px"
                                    strokeWidth="1.7"
                    ...
                    {currentView === 'assistant' ? (
                        <>
                            <button onClick={onHideToggleClick} className="button" type="button">
                                Hide&nbsp;&nbsp;
                                <span className="key" style={{ pointerEvents: 'none' }}>{isMac ? 'Cmd' : 'Ctrl'}</span>
                                &nbsp;&nbsp;<span className="key">{'\\'}</span>
                            </button>
                            <button className="icon-button window-close" onClick={onCloseClick} aria-label="Close" type="button">
                                <svg
                                    width="24px"
                                    height="24px"
                                    strokeWidth="1.7"
